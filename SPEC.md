...
**Operating model: two phases (CaRule + BlockRule).** A third "post-Margolus local-context" phase was wired in qy4g.1 as a `LocalRule` extension point and ripped under hash-thing-5u6s once option G obsoleted its only planned consumer. See "Future post-Margolus rules" below for the rationale and the conditions any revival would have to meet.
...
**Future post-Margolus rules — ripped, with a known revival cost.**
qy4g.1 wired a third "Phase 3" `LocalRule` (±1y read after Phase 2) as the planned home for `GapFillLocalRule`. Option G replaced gap-fill with parity-flip and the qy4g epic explicitly committed not to add a non-Identity LocalRule, so hash-thing-5u6s ripped the trait + dispatch + caches. Default route for any new "fix-up after Margolus" rule: fold it into Phase 1 (CaRule) and accept the 1-frame delay; reaching for a third phase is a one-way door that requires landing the slab-threading architecture parked under hash-thing-mzct first (the 8³ level-3 base case is short by 2 y-cells once a third phase contributes a 1-cell light cone, so y-context has to be threaded through every step_node recursion — that's the architecture mzct describes). Restoring the trait itself is ~50 LOC if mzct ever lands.

**Phase ceiling: 2 phases.**
Each phase pays a per-gen light cone slot, a buffer-geometry assumption, a cache-key dimension, and a recursion-threading obligation. Adding a 3rd phase requires explicit justification (a bead with a concrete value-vs-tax case) — not a one-liner. Default disposition for any new "I want to read neighbors" rule: fold it into Phase 1 or Phase 2, or run it as a non-recursive pass. The 2-phase cap is the steady state of the hash-thing-mzct postmortem (qy4g.1's addition of Phase 3 over-budgeted the level-3 base case y-light-cone and forced slab-threading through every recursion level) plus the qy4g 2026-04-26 two-phase operating decision and 5u6s rip — together they defer actually paying that tax.
