...
mod tests {
    ...
    //! the `Node` level/emptiness/side_len accessors, and the `Cell`
    //! packed-encoding contract.
    //!
    //! The octant helpers are load-bearing: `octant_index` is called on
    //! every subtree build in `store.rs` (set_cell, from_flat_recursive,
    ...
    // ---- Cell encoding tests (hash-thing-1v0.1) ----
