...
- **9k4w.2 empty-cell bitmask (1.26× claim).** Implemented by storing grandchildren occupancy in each interior node. Nodes are now 44 bytes (11 u32s), up from 36 bytes; the accepted +22% memory tradeoff buys a full 64-bit mask rather than only the already-landed 8-bit immediate-child mask. **Most likely outcome: ~1.3× with full bitmask + memory bloat.**
