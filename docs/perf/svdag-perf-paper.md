...
| 2026-04-25 | ember | New companion doc `docs/perf/cswp-lod.md` (bead `hash-thing-cswp.6`). Memory/streaming LOD policy for 4096³+ worlds. Distinguishes existing render LOD (already shipped, pixel-projected subtree skip) from memory LOD (memory-resident granularity); proposes log₄ chunk-radius → LOD-level mapping; documents material-collapse policy options (status-quo "largest populated child" recommended as default, fixed per-material priority flagged for edward as the second-place candidate); recommends hard-swap transitions over crossfade. Re-derives 5.1/5.2/5.3 budgets and walks back the §4.7.4 "8192³ streaming threshold" framing: with the proposed LOD layer, **streaming-plus-LOD becomes mandatory at 16384³+**, not 8192³. The LOD layer's value below that is bounded per-chunk cost for cswp.5's streaming bookkeeping, not absolute memory shrinkage. No code change. Doc-only design; impl lives in a follow-up sub-bead. |
