...
fn repeated_lod_recompute_with_periodic_compaction_bounds_growth() {
    ...
    // policy, and triggers compaction whenever the store-growth ratio
    // crosses `LOD_COMPACT_RATIO_THRESHOLD`. The cswp.8.3 lex-loop
    // accrued ghost chains and reliably tripped the trigger after a
    // handful of crossings; the cswp.8.3.1 single-pass descent collapses
    // uniform subtrees in one hash-cons-friendly call, so peak growth
    // can stay below the trigger entirely. Both outcomes are valid: the
    // contract tested here is "peak stays bounded across `recompute()`
    // calls", with compaction acting as a backstop only when needed.
    let mut world = make_4x4x4_chunk_world();
    // Plant some far cells so the policy produces non-trivial collapses
    // (otherwise the empty-world fast path masks any node accrual).
    for cx in 0..4 {
        for cy in 0..4 {
            world.set(
    ...
    // If compaction did fire, defend against a regression where
    // `compact_keeping` retains so many extra roots that nothing is
    // reclaimed (the peak-bounded check would still pass).
    let mut observed_shrink = false;

    // 64 player-chunk crossings — long enough to exercise the trigger
    // multiple times in the lex-loop world; under the descent it usually
    // stays below the trigger entirely.
    ...
    // Peak growth must stay bounded. The trigger fires at >4×; allow up
    // to 8× as headroom for the inter-trigger window. Anything beyond
    // means either compaction never fired *and* growth went unbounded
    // (descent regression), or compaction fired but didn't shed nodes
    // (compact_keeping regression).
    ...
    // If compaction fired at all, at least one call must have shed
    // nodes. (When compactions == 0 the descent kept growth bounded on
    // its own — the cswp.8.3.1 happy path; nothing further to assert.)
    if compactions > 0 {
        assert!(
            observed_shrink,
            "compaction fired ({compactions}×) but never shrank the store \
